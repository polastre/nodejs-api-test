...
function getDistance (params, cb) {
  ...
  if (_.isUndefined(params.ip_address)) {
    return cb(422, { error: 'IP Address is required.'} );
  }
  if (_.isUndefined(params.first_name)) {
    return cb(422, { error: 'First Name is required.'} );
  }
  if (_.isUndefined(params.last_name)) {
    return cb(422, { error: 'Last Name is required.'} );
  }
  // Get the lat/long coordinates of the IP address
  var geo = geoip.lookup(params.ip_address);
  if (_.isNull(geo)) {
    return cb(422, { error: 'Unable to lookup coordinates for IP address.'});
  }
